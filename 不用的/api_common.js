...
console.log("global_type :: ", global_type);
if(global_type == GlobalConfig.TYPE_PAGE_IMAGE_TO_VIDEO){
    console.log("图片转视频");
}else if(global_type == GlobalConfig.TYPE_PAGE_TEXT_TO_VIDEO){
    console.log("文本转视频");
}else if(global_type == GlobalConfig.TYPE_PAGE_TEMPLATE_TO_VIDEO){
    console.log("模板转视频");
}else if(global_type == GlobalConfig.TYPE_PAGE_IMAGE_TO_IMAGE){
    console.log("图片转图片");
}else if(global_type == GlobalConfig.TYPE_PAGE_TEXT_TO_IMAGE){
    console.log("文本转图片");
}
...
id_btn_generate.addEventListener('click', function(){
    ...
    setGenerateButtonState(false);
     ...
     if (global_type == GlobalConfig.TYPE_PAGE_IMAGE_TO_VIDEO){
        ...
        if(model == "Google VOE3"){
            startGenerateVideoByVeo3  (GlobalConfig.TYPE_IMAGE_FLUX); // flux类型 
        }else if(model == "AutoGen"){
            startGenerateVideoByV  (GlobalConfig.TYPE_VIDEO_V_IMAGE_TO_VIDEO); // nano类型
        }
     }else if (global_type == GlobalConfig.TYPE_PAGE_TEXT_TO_VIDEO){
        var model = id_btn_model.innerText;
        if(model == "Google VOE3"){
        ...
        startGenerateImage  (GlobalConfig.TYPE_IMAGE_FLUX); // flux类型 
     } else if (global_type == GlobalConfig.TYPE_PAGE_IMAGE_TO_IMAGE){
        startGenerateImage  (GlobalConfig.TYPE_IMAGE_FLUX); // flux类型 
     } else if (global_type == GlobalConfig.TYPE_PAGE_TEXT_TO_IMAGE){
        startGenerateImage  (GlobalConfig.TYPE_IMAGE_NANO); // nano类型
 ...
 */
// veo3 只能文生视频
async function startGenerateVideoByVeo3 () {
    // 仅图片生成视频面板
    if (!isLoggedIn) { 
        window.open('/login', '_self');
        return; 
    ...
    try {
        ...
        let jsonData = {
                        "opt": "0", // veo3没作用
                        "prompt": prompt,
                        "model": "0", // 0:veo3  1:veo3-fast 2:veo3-pro
                        "uid": localUser ? localUser.uid : 0,
                        "project_id": GlobalConfig.project_id,
                        "product_id": GlobalConfig.product_id,
                        "device_info": "6789",
...
async function startGenerateImage (image_type) {
    // 仅图片生成视频面板
    if (!isLoggedIn) { 
        window.open('/login', '_self');
        return; 
    ...
    try {
        ...
        if (image_type == GlobalConfig.TYPE_IMAGE_NANO) {
            ...
            jsonData["prompt"] = prompt;
            jsonData["model"] = "flux-kontext-pro";
